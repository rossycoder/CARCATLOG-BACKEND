...
class VehicleController {
  ...
  async searchCars(req, res, next) {
    try {
      console.log('[Vehicle Controller] Search request received with query:', req.query);
      ...
      console.log('[Vehicle Controller] Built MongoDB query:', JSON.stringify(query, null, 2));

      // Exact match filters
      if (gearbox) query.transmission = new RegExp(gearbox, 'i');
      if (bodyType) query.bodyType = new RegExp(bodyType, 'i');
      if (colour) query.color = new RegExp(colour, 'i');
      if (doors) query.doors = parseInt(doors);
      if (seats) query.seats = parseInt(seats);
      if (fuelType) query.fuelType = new RegExp(fuelType, 'i');
      if (engineSize) {
        // Handle engine size ranges
        const engineSizeNum = parseFloat(engineSize);
        if (engineSize === '3.0+') {
          query.engineSize = { $gte: 3000 };
        } else {
          query.engineSize = { $lte: engineSizeNum * 1000 };
        }
      }
      ...
      console.log('[Vehicle Controller] Search results:', {
        total,
        returned: cars.length,
        sampleMileages: cars.slice(0, 5).map(c => ({ id: c._id, mileage: c.mileage }))
      });
